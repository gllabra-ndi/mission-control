...
type GrowthSegment = "existing" | "new";
...
export function ProjectsBacklogGrowth({ tasks, projectOptions }: ProjectsBacklogGrowthProps) {
    ...
    const monthlyData = useMemo(() => {
        ...
        return monthStarts.map((monthStartMs) => {
            ...
            projectTasks.forEach((task) => {
                ...
                } else if (createdMs < monthEndExclusiveMs) {
                    newAdded += effort;
    ...
    const maxTotal = Math.max(1, ...monthlyData.map((row) => row.totalBacklog));
            ...
            <div className="border border-border/50 bg-surface/20 rounded-xl overflow-hidden">
                ...
                <div className="p-5">
                    <div className="flex items-center gap-5 text-xs text-text-muted mb-4">
                        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#1f3b73]" /> Existing backlog</div>
                        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#69a8ff]" /> New work added in month</div>
                    </div>
                                    ...
                                    <div className="w-12 h-[240px] border border-border/40 rounded overflow-hidden flex flex-col justify-end bg-background/50">
                                        ...
                                        {row.newAdded > 0 && (
                                            <button
                                                ...
                                                title={`${row.monthYearLabel} • New work added in month • ${row.newAdded.toFixed(1)}h`}
                                            />
                                        )}
                                    </div>
                                    <div className="mt-2 text-xs text-text-main">{row.monthLabel}</div>
                                    <div className="text-[10px] text-text-muted">{row.totalBacklog.toFixed(0)}h</div>
                                </div>
                            );
                        })}
                    </div>
